...
func GetPendingPickupRequestsHandler(w http.ResponseWriter, r *http.Request) {
	...
	requests, err := (*database).GetPendingPickupRequests(distributorID)
	if err != nil {
		log.Error("failed to fetch pending pickup requests: ", err)
		api.InternalErrorHandler(w)
		return
	}
